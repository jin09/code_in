...
void printZigZag(binaryTreeNode* root){
    ...
    while(!q.empty()){
        ...
        if(temp == NULL){
            ...
            //push children of node to the queue and the node itself to the stack.
            while(temp != NULL){
                if(temp->left != NULL){
                    q.push(temp->left);
                }
...
cout<<endl;
cout<<"Printing in zig zag order ----"<<endl;
printZigZag(root1);
return 0;
}
