/*   print all the root to leaf paths in the tree.
...
void printAllRootToLeafPath(binaryTreeNode* root){
    static vector<int> v;
    if(root == NULL){
        return ;
    }
    ...
    if(root->left != NULL){
        printAllRootToLeafPath(root->left);
    }
    if(root->right != NULL){
        printAllRootToLeafPath(root->right);
    }
    v.pop_back();
}
