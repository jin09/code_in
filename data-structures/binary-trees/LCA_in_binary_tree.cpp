/*    Lowest common ancestor in the binary tree
      Simple O(n) approach where I save the path of both the nodes and try to find their intersection point
*/
...
int lowestCommonAncestorInBinaryTree(binaryTreeNode* root, int low, int high){
    if(root == NULL){
        return -1;
    }
    vector<int> path1, path2;
    //store path of each node in their respective vectors
    vectorOfPathFromRootToGivenNode(root,low,path1);
    vectorOfPathFromRootToGivenNode(root,high,path2);
...
cin>>high;
cout<<"Lowest common ancestor in the binary tree is : "<<lowestCommonAncestorInBinaryTree(root,low,high);
