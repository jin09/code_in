/*   Print tree vertical order (hashing is used)

INPUT:

              ___________1__________
             /                      \
        _____2____              _____3____
       /          \            /          \
       4           5_          6_          7
                     \           \
                      8           9
...
int maxKey = -100;
int minKey = 100;
void fillMapVerticalOrder(binaryTreeNode* root, int column, unordered_map<int, vector<int> > & m){
    if(root == NULL){
        return;
    }
    maxKey = max(maxKey,column);
    minKey = min(minKey,column);
    m[column].push_back(root->data);
    fillMapVerticalOrder(root->left,column-1,m);
    fillMapVerticalOrder(root->right,column+1,m);
}

int main(){
binaryTreeNode* root = takeInputLevelOrder();
printPretty(root,2,3,cout);
unordered_map<int, vector<int> > mymap;

fillMapVerticalOrder(root,0,mymap);
