/*    Replace all the nodes with the sum of all the nodes that are greater than the node itself..
...
int replaceNodesWithSumOfGreaterValue(binaryTreeNode* root , int data){
    if(root == NULL){
        return data;
    }
    int right = replaceNodesWithSumOfGreaterValue(root->right,data);
    int sum = root->data + right;
    root->data = right;
    int left = replaceNodesWithSumOfGreaterValue(root->left,sum);
    return left;
}
...
replaceNodesWithSumOfGreaterValue(root,0);
printPretty(root,3,4,cout);
return 0;
}
