/*    traverse the tree and duplicate each node, each duplicated node should be inserted to the left of the orginal node.
...
void insertDuplicateNodes(binaryTreeNode* root){
    if(root==NULL){
        return ;
    }
    ...
    insertDuplicateNodes(duplicate->left);
    insertDuplicateNodes(right);
}
