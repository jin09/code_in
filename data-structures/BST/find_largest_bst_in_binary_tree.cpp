...
nodeBoolsize findLargestBSTInBinaryTree(binaryTreeNode* root){
    nodeBoolsize b;
    if(root == NULL){
        b.ans = true;
        b.temp = NULL;
    ...
    nodeBoolsize left = findLargestBSTInBinaryTree(root->left);
    nodeBoolsize right = findLargestBSTInBinaryTree(root->right);

    if(left.temp == NULL && right.temp == NULL){
        b.ans = true;
        b.temp = root;
        b.sizeOfTree = b.sizeOfTree +1;
        return b;
    }

    if(left.ans == true && right.ans == true){
        if(root->data > left.temp->data && root->data < right.temp->data){
            b.ans = true;
            b.temp = root;
            b.sizeOfTree = left.sizeOfTree + right.sizeOfTree + 1;
            return b;
        }
        else{
            b.ans = false;
            if(left.sizeOfTree > right.sizeOfTree){
                b.temp = left.temp;
                b.sizeOfTree = left.sizeOfTree;
            }
            else{
                b.temp = right.temp;
                b.sizeOfTree = right.sizeOfTree;
            }
            return b;
        }
    }
...
nodeBoolsize b = findLargestBSTInBinaryTree(root);
printPretty(b.temp,3,4,cout);
